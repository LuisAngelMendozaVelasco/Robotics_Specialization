# Week 1

Welcome to Robotics Capstone! This week you will choose between two tracks available to you for your capstone. Please make sure you watch the videos carefully to make the choice. In the MIP track, you will learn how to use MATLAB (your numerical tool for this capstone track) to simulate dynamical systems numerically.In the AR track, you will learn to use the rover simulator, purchase the kit and implement Dijkstra's algorithm in python.

- [Video - Capstone Introduction and Choosing the Capstone Project](https://www.coursera.org/learn/robotics-capstone/lecture/dO6jR/capstone-introduction-and-choosing-the-capstone-project)

- [Video - Introduction to the Mobile Inverted Pendulum (MIP) Track](https://www.coursera.org/learn/robotics-capstone/lecture/fWzEv/introduction-to-the-mobile-inverted-pendulum-mip-track)

- [Video - Introduction to the Autonomous Rover (AR) Track](https://www.coursera.org/learn/robotics-capstone/lecture/Hkgbu/introduction-to-the-autonomous-rover-ar-track)

# Week 1: Lesson Choices

## MIP Track: Using MATLAB for Dynamic Simulations

- [Video - A1.1 Using MATLAB for Dynamic Simulations](https://www.coursera.org/learn/robotics-capstone/lecture/2F9q2/a1-1-using-matlab-for-dynamic-simulations)

## AR Track: Dijkstra's and Purchasing the Kit

- [Video - (Review) Dijkstra's Algorithm](https://www.coursera.org/learn/robotics-capstone/lecture/VFOJO/review-dijkstras-algorithm)

- [Reading - B1.1 Purchasing the Robot Kit](https://www.coursera.org/learn/robotics-capstone/supplement/bobqE/b1-1-purchasing-the-robot-kit)

- [Reading - B1.2 The Rover Simulator](https://www.coursera.org/learn/robotics-capstone/supplement/bz5fC/b1-2-the-rover-simulator)